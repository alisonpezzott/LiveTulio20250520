...
def set_mode_cli():
    """
    Set mode to command_line
    """
    run_fab_command(command='config set mode command_line')
...
def auth_login():
    ...
    if client_id and client_secret and tenant_id:
        try:
            ...
            print("Authenticating with interactive login")
            run_fab_command(command='config set fab_encryption_fallback_enabled true')
            run_fab_command(command='auth login')
...
def auth_logout():
    """
    Auth logout with browser
    """
    run_fab_command(command='auth logout')
...
def export_item(
    ...
    """
    Exports a item from workspace.
    Args:
        src_path (str): The source path of the item to be exported.
        workspace_name (str): The name of the workspace from where the item will be exported.
        item_type (str, optional): The type of the item. If not provided, it will be inferred from the platform data.
        item_name (str, optional): The name of the item. If not provided, it will be inferred from the platform data.
    Returns:
        str: Not defined yet
    ...
    if not what_if:
        run_fab_command(
            f"export /{workspace_name}.workspace /{item_name}.{item_type} -o {src_folder} -f"
        )

        # Return id after export
...
def list_items(workspace_name):
    """
    Lists items in a workspace.
    Args:
        workspace_name (str): The name of the workspace to list items from.
        item_type (str, optional): The type of the item. If not provided, it will be inferred from the platform data.
        item_name (str, optional): The name of the item. If not provided, it will be inferred from the platform data.
    Returns:
        str: Not defined yet
    ...
    items = run_fab_command(
        f"ls /{workspace_name}.workspace/ -l"
    )
